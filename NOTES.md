...
| 20 | Theme (Step 12) | `progressBarRadius = 18` on the 8 pt progress bar, on the theory that any radius above half the height renders as a pill | Core Animation doesn't clamp an oversized `cornerRadius`: the orange loader's leading and trailing ends drew flattened and square-looking instead of round | Token removed; `ProgressBarView` sets the track's and fill's radius to `bounds.height / 2` in its own `layoutSubviews`. Spec 05 §1 and the progress bar spec updated | me — noticed the sharp ends on the loading screen |
